require([
  ...
  let info = new OAuthInfo({
    ...
    popupCallbackUrl: "https://derekjuracek.github.io/Subways_Navigator_AD_Sign_In/oauth-callback.html", // make sure to set callback page to one that supports authentication type
  });
  ...
  // Delay for 5 seconds to allow the user to authenticate

  // Try to get the user's credentials. This will prompt the user to log in if they're not already signed in.
  IdentityManager.getCredential(
    `https://mtagisdev.lirr.org/dosportaldev/sharing/rest`
  )
    .then((credential) => {
      console.log("User is signed in.");
      console.log(credential);
      console.log(credential.token);
      runApp(credential.token); // Here's the access token
    })
    .catch((error) => {
...
`;
    ...
    const print = new Print({
      view,
      printServiceUrl: `https://mtagisdev.lirr.org/dosserverdev/rest/services/PrintTemplates/ExportWebMap/GPServer/Export%20Web%20Map`,
